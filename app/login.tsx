...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const daysLeftPremium = (premiumExpiresAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);

      const isMonthly = daysLeftPremium > 0 && daysLeftPremium <= 31;
      const isHalfYear = daysLeftPremium > 31 && daysLeftPremium <= 186;
      ...
      if (isPremium || !trialExpired) {
        console.log(`✅ 유효한 이용권: ${
          isMonthly ? '1개월' : isHalfYear ? '6개월' : isYearly ? '1년' : '기타'
        } 남음`);
